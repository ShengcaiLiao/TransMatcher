...
def main(args):
    ...
    if ibn_type == 'none':
        ibn_type = None
    model = restranmap.create(args.arch, ibn_type=ibn_type, final_layer=args.final_layer, neck=args.neck, nhead=args.nhead, 
                num_encoder_layers=args.num_trans_layers - 1, dim_feedforward=args.dim_feedforward).cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_trans_layers', type=int, default=3,
                        help="the number of decoder layers (default=3). The number of encoder layers is num_trans_layers-1.")
